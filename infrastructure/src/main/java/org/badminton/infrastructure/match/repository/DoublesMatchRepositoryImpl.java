...
import static org.badminton.domain.domain.match.entity.QDoublesMatch.*;
...
public class DoublesMatchRepositoryImpl implements DoublesMatchRepositoryCustom {
	...
	public List<DoublesMatch> findAllCompletedByClubMemberId(Long clubMemberId) {
		...
		return queryFactory
			...
			.on(doublesMatch.team1.leagueParticipant1.leagueParticipantId.eq(leagueParticipant1.leagueParticipantId))
			.join(leagueParticipant2)
			.on(doublesMatch.team1.leagueParticipant2.leagueParticipantId.eq(leagueParticipant2.leagueParticipantId))
			.join(leagueParticipant3)
			.on(doublesMatch.team2.leagueParticipant1.leagueParticipantId.eq(leagueParticipant3.leagueParticipantId))
			.join(leagueParticipant4)
			.on(doublesMatch.team2.leagueParticipant2.leagueParticipantId.eq(leagueParticipant4.leagueParticipantId))
			.where(
