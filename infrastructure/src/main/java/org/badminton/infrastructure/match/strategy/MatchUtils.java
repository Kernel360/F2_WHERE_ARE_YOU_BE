...
public class MatchUtils {
	...
	public static Integer calculateNextRoundMatchId(int currentMatchId, int totalParticipants, int startMatchId) {
		...
		for (int level = 1; level <= totalRounds; level++) {
			if (currentMatchId < currentRoundOffset
				...
				int nextRoundOffset =
					currentRoundOffset + matchesInCurrentLevel; // 현재 매치의 인덱스 + 매치 수 -> 다음 라운드의 첫번째 매치의 ID
				return nextRoundOffset
					+ nextMatchIndexInLevel; // 현재 매치의 다음 매치의 ID 는 다음 라운드의 첫번 째 MATCH ID + 현재 매치의 인덱스 / 2
			}
